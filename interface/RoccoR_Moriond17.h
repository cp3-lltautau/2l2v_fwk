...
struct CrystalBall_Moriond17{
    ...
    CrystalBall_Moriond17(){
	init(0, 1, 10, 10);
    }
    CrystalBall_Moriond17(double m_, double s_, double a_, double n_){
	init(m_, s_, a_, n_);
    }
...
const double CrystalBall_Moriond17::pi    = TMath::Pi();
const double CrystalBall_Moriond17::SPiO2 = sqrt(TMath::Pi()/2.0);
const double CrystalBall_Moriond17::S2    = sqrt(2.0);


class RocRes_Moriond17{
    private:
	...
	CrystalBall_Moriond17  cb[NMAXETA][NMAXTRK];
...
class RocOne_Moriond17{
	...
	RocOne_Moriond17(std::string filename, int iTYPE=0, int iSYS=0, int iMEM=0);
	...
	double kGenSmear(double pt, double eta, double v, double u, RocRes_Moriond17::TYPE TT=RocRes_Moriond17::Data) const;
	...
	RocRes_Moriond17& getR() {return RR;}
};


class RoccoR_Moriond17{
    public:
	RoccoR_Moriond17();
	RoccoR_Moriond17(std::string dirname);
	~RoccoR_Moriond17();

	void init(std::string dirname);

	double kGenSmear(double pt, double eta, double v, double u, RocRes_Moriond17::TYPE TT=RocRes_Moriond17::Data, int s=0, int m=0) const;
    ...
    private:
	std::vector<std::vector<RocOne_Moriond17> > RC;
};
