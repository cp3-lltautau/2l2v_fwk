...
int RocRes_Moriond17::getBin(double x, const int NN, const double *b) const{
    for(int i=0; i<NN; ++i) if(x<b[i+1]) return i;
    return NN-1;
}

RocRes_Moriond17::RocRes_Moriond17(){
    reset();
}

void RocRes_Moriond17::reset(){
    NETA=1;
    NTRK=1;
    NMIN=1;
...
int RocRes_Moriond17::getEtaBin(double feta) const{
    return getBin(feta,NETA,BETA);
}

int RocRes_Moriond17::getNBinDT(double v, int H) const{
    return getBin(v,NTRK,dtrk[H]);
}

int RocRes_Moriond17::getNBinMC(double v, int H) const{
    return getBin(v,NTRK,ntrk[H]);
}

void RocRes_Moriond17::dumpParams(){
    using namespace std;
...
void RocRes_Moriond17::init(std::string filename){
...
double RocRes_Moriond17::Sigma(double pt, int H, int F) const{
    double dpt=pt-45;
    return rmsA[H][F] + rmsB[H][F]*dpt + rmsC[H][F]*dpt*dpt;
}

double RocRes_Moriond17::getUrnd(int H, int F, double w) const{
    return ntrk[H][F]+(ntrk[H][F+1]-ntrk[H][F])*w;
}

double RocRes_Moriond17::kSpread(double gpt, double rpt, double eta, int n, double w) const{
...
double RocRes_Moriond17::kSmear(double pt, double eta, TYPE type, double v, double u) const{
    int H = getBin(fabs(eta), NETA, BETA);
...
double RocRes_Moriond17::kSmear(double pt, double eta, TYPE type, double w, double u, int n) const{
    int H = getBin(fabs(eta), NETA, BETA);
    int F = n>NMIN ? n-NMIN : 0;
...
double RocRes_Moriond17::kExtra(double pt, double eta, int n, double u, double w) const{
    int H = getBin(fabs(eta), NETA, BETA);
    int F = n>NMIN ? n-NMIN : 0;
...
const double RocOne_Moriond17::MPHI=-TMath::Pi();

int RocOne_Moriond17::getBin(double x, const int NN, const double *b) const{
    for(int i=0; i<NN; ++i) if(x<b[i+1]) return i;
    return NN-1;
}

int RocOne_Moriond17::getBin(double x, const int nmax, const double xmin, const double dx) const{
    int ibin=(x-xmin)/dx;
    if(ibin<0) return 0;
...
RocOne_Moriond17::RocOne_Moriond17(){
    reset();
}

RocOne_Moriond17::RocOne_Moriond17(std::string filename, int iTYPE, int iSYS, int iMEM){
    init(filename, iTYPE, iSYS, iMEM);
}


bool RocOne_Moriond17::checkSYS(int iSYS, int iMEM, int kSYS, int kMEM){
    if(iSYS==0 && iMEM==0)	      return true;
    if(iSYS==kSYS && iMEM==kMEM)      return true;
    return false;
}

bool RocOne_Moriond17::checkTIGHT(int iTYPE, int iSYS, int iMEM, int kTYPE, int kSYS, int kMEM){
...
void RocOne_Moriond17::init(std::string filename, int iTYPE, int iSYS, int iMEM){
...
double RocOne_Moriond17::kScaleDT(int Q, double pt, double eta, double phi) const{
...
double RocOne_Moriond17::kScaleMC(int Q, double pt, double eta, double phi, double kSMR) const{
    int H=getBin(eta, NETA, BETA);
    int F=getBin(phi, NPHI, MPHI, DPHI);
    double m=M[MC][H][F];
    double a=A[MC][H][F];
...
double RocOne_Moriond17::kScaleAndSmearMC(int Q, double pt, double eta, double phi, int n, double u, double w) const{
    double k=kScaleMC(Q, pt, eta, phi);
    return k*RR.kExtra(k*pt, eta, n, u, w);
}


double RocOne_Moriond17::kScaleFromGenMC(int Q, double pt, double eta, double phi, int n, double gt, double w) const{
    double k=kScaleMC(Q, pt, eta, phi);
    return k*RR.kSpread(gt, k*pt, eta, n, w);
}


double RocOne_Moriond17::kGenSmear(double pt, double eta, double v, double u, RocRes_Moriond17::TYPE TT) const{
...
RoccoR_Moriond17::RoccoR_Moriond17(){}

RoccoR_Moriond17::RoccoR_Moriond17(std::string dirname){
    init(dirname);
}


void
RoccoR_Moriond17::init(std::string dirname){
	...
	for(int m=0; m<sn; ++m){
		...
		v.push_back(RocOne_Moriond17(Form("%s/%d.%d.txt", dirname.c_str(),0,0),0,0,0));
	    }
	    else{
		v.push_back(RocOne_Moriond17(inputfile, 0, si, m));
	    }
	}
	RC.push_back(v);
    }
...
double RoccoR_Moriond17::kGenSmear(double pt, double eta, double v, double u, RocRes_Moriond17::TYPE TT, int s, int m) const{
    return RC[s][m].kGenSmear(pt, eta, v, u, TT);
}

double RoccoR_Moriond17::kScaleDT(int Q, double pt, double eta, double phi, int s, int m) const{
    return RC[s][m].kScaleDT(Q, pt, eta, phi);
}

double RoccoR_Moriond17::kScaleAndSmearMC(int Q, double pt, double eta, double phi, int n, double u, double w, int s, int m) const{
    return RC[s][m].kScaleAndSmearMC(Q, pt, eta, phi, n, u, w);
}

double RoccoR_Moriond17::kScaleFromGenMC(int Q, double pt, double eta, double phi, int n, double gt, double w, int s, int m) const{
