...
if [[ $step > 0.999 &&  $step < 2 ]]; then
   if [[ $step == 1 || $step == 1.0 ]]; then        #submit jobs for 2l2v analysis
	echo "JOB SUBMISSION"
	runAnalysisOverSamples.py -e runZHllvvAnalysis -j $JSON -o $RESULTSDIR  -c $MAINDIR/../runAnalysis_cfg.py.templ -p "@data_pileup="$pileup" @jacknife=0 @saveSummaryTree=True @runSystematics=False @useMVA=True @jacks=0" -s $queue --report True --key zhllvv_2016 $arguments
   fi
fi
...
if [[ $step > 2.999 && $step < 4 ]]; then
	...
	runPlotter --iEcm 13 --iLumi $INTLUMI --inDir $RESULTSDIR/ --outFile ${PLOTTER}.root  --json $JSON --noPlot --fileOption RECREATE --key  zhllvv_2016  $arguments       
        cp  ${PLOTTER}.root  ${PLOTTER}_MCOnly.root

	runPlotter --iEcm 13 --iLumi $INTLUMI --inDir $RESULTSDIR/ --outFile ${PLOTTER}.root  --json $JSON --noPlot --fileOption UPDATE   --key  zhllvv_2016                       $arguments 
	ln -s -f ${PLOTTER}.root plotter.root 
    fi        
   
    if [[ $step == 3 || $step == 3.1 ]]; then  # make plots and combine root files for zhlvv_2016 study    
	runPlotter --iEcm 13 --iLumi $INTLUMI --inDir $RESULTSDIR/ --outDir $PLOTSDIR/zhlvv_2016/ --outFile ${PLOTTER}.root  --json $JSON --no2D --plotExt .png --plotExt .pdf  --key zhllvv_2016 $arguments
    fi


    if [[ $step == 3 || $step == 3.11 ]]; then  # make plots and combine root files for zhlvv_2016 study    
        runPlotter --iEcm 13 --iLumi $INTLUMI --inDir $RESULTSDIR/ --outDir $PLOTSDIR/zhlvv_2016/ --outFile ${PLOTTER}.root  --json samples.json --no2D --plotExt .png --plotExt .pdf  --key zhllvv_2016 --fileOption READ --showUnc 0.0 --only "emu(|eq0jets|geq1jets|vbf)_mt_(In|Out)b.*" --rebin 3  --noLog --removeRatioPlot  $arguments 
    fi
fi
