...
## Run 2l2vAnalysis over samples
    ...
    printf "\n\t%-5s  %-40s\n"  "1"  "run 'runZHllvvAnalysis' on all samples" 
    printf "\n\t%-5s  %-40s\n"  "1.0"  "make plots for mcbased analysis"
    printf "\n\t%-5s  %-40s\n"  "1.1"  "run 'runZHllvvAnalysis' on photon samples" 
    printf "\n\t%-5s  %-40s\n"  "1.2"  "run 'runZHllvvAnalysis' on photon samples with photon re-weighting"

## Run photonZ closure tests:
    printf "\n\t%-5s  %-40s\n"  "1.3"  "run 'runZHllvvAnalysis' on photon and DY samples in MC only"
    printf "\n\t%-5s  %-40s\n"  "1.4"  "run 'runZHllvvAnalysis' on photon samples with photon re-weighting with MC weights"  
...
if [[ $step > 0.999 &&  $step < 2 ]]; then
   if [[ $step == 1 || $step == 1.0 ]]; then        #submit jobs for 2l2v analysis
	echo "JOB SUBMISSION"
	runAnalysisOverSamples.py -e runZHllvvAnalysis -j $JSON -o $RESULTSDIR  -c $MAINDIR/../runAnalysis_cfg.py.templ -p "@data_pileup="$pileup" @jacknife=0 @saveSummaryTree=True @runSystematics=False @useMVA=True @jacks=0" -s $queue --report True --key zhllvv_2015 $arguments
   fi
...
if [[ $step > 2.999 && $step < 4 ]]; then
	...
	echo "MAKE SUMMARY ROOT FILE, BASED ON AN INTEGRATED LUMINOSITY OF $INTLUMI"
        runPlotter --iEcm 13 --iLumi $INTLUMI --inDir $RESULTSDIR/ --outFile ${PLOTTER}.root  --json $JSON --plotExt .png --plotExt .pdf -fileOption UPDATE   --key zhllvv_2015  $arguments 
